...
class Options_Framework_Media_Uploader {
	...
	static function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
		...
		$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" placeholder="' . __('Dosya seçilmedi', 'theme-textdomain') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'Yükle', 'theme-textdomain' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( 'Sil', 'theme-textdomain' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . __( 'Bu özelliği kullanmak için lütfen WordPress sürümünüzü yükseltin.', 'theme-textdomain' ) . '</i></p>';
		}
		...
		if ( $value != '' ) {
			$remove = '<a class="remove-image">Sil</a>';
	...
	function optionsframework_media_scripts( $hook ) {
		...
		wp_localize_script( 'of-media-uploader', 'optionsframework_l10n', array(
			'upload' => __( 'Yükle', 'theme-textdomain' ),
			'remove' => __( 'Sil', 'theme-textdomain' )
		) );
	}
}